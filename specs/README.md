...
|---|---|---|---|
| 1 | 002, 003, 004 | A single node serves clone, fetch, and push with the log as the source of truth | built; 002 and 003 complete, 003 closed on 2026-09-11 when spec 021's live run carried its 51 cases; 004 at `testing` on two tests nobody has written and the Spaces probe |
...
| `TestE2EHundredConcurrentPushesFromEightClients` and `TestSlowMaterializeTenThousandEntries` | 004 | a builder writing them and the `e2e` and `e2e-slow` jobs running them; `grep -r` finds neither in the tree |
...
| the 40 second fuzz search | 009, 016 | the `fuzz` job of `verify.yml`, which now runs on a dispatch as well as on the Sunday cron since `7e7b1cd`; the dispatched run 34598740040 reports it `success` |
...
| a release that publishes on a repository other than this one, which the criterion calls the fork tag | 017 | a change to `release.yml` first. `ORIGOD_IMAGE` and `STUBS_IMAGE` are fixed to `ghcr.io/latere-ai/...` at lines 38 and 39 with no `vars` fallback and are read at sixteen sites, so a fork's tag pushes to this organization's packages and fails before publishing. Reading them from repository variables, defaulting to `ghcr.io/${{ github.repository }}`, is what makes the fork tag runnable at all |
