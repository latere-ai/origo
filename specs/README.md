...
| the per-subject request rate is `ORIGO_REQUESTS_PER_MINUTE`, 600 by default, and every response of the surface carries the figure in force for its own subject as `RateLimit-Limit` (the IETF draft field): the authorizer's `requests_per_minute` where it named one, the variable otherwise, and no header when the limit is off; the `kind` overlay runs at 6000 and each cluster scenario mints a subject of its own, because the scenarios drive one node far harder than any caller of a live installation, and 021's `rate_limited` case reads the header rather than assuming the default | 012 | 002, 013, 003, 021 |
...
| 021's `rate_limited` case sends at most twice the figure the header names, the bound the drain inequality gives for a runner that sends twice as fast as one node refills, and records the assertion in `Report.Unverified` when nothing refused inside it, because a balancer gives one subject a bucket a node and no bound converges there; the stub and the stack converge inside `limit + 1` and still require that list empty | 021 | 012, 002, 013 |
