...
four variables of the `origo` binary in a table of its own, because
...
reduction of the deck's 91 `depends_on` edges: an arrow is drawn only
where no other path already carries it, which leaves 33. Reachability is
...
|---|---|---|---|
| 1 | 002, 003, 004 | A single node serves clone, fetch, and push with the log as the source of truth | built; 002 and 003 complete, 003 closed on 2026-09-11 when spec 021's live run carried its 51 cases; 004 complete, closed on 2026-09-11 on the hundred-push test, the ten-thousand entry scenario of job 103373438650, and the Spaces probe |
...
| 12 | 025 | An agent reads a file, finds one by name, opens a change, and pushes a commit through a command it pipes into a shell instead of a clone or a hand-written HTTP call | 025 built and at testing: `internal/origoclient`, which speaks the contract and formats nothing, `internal/origocli` and the twelve commands on it, `cmd/origo` as a second binary, `docs/cli.md` whose blocks are its own test, and `skills/origo/SKILL.md`, whose resident cost is 99 bytes against the 5 725 the eight read tools of the MCP shape would hold, all in the tree; the release carries eight archives rather than four and `release-verify` downloads both patterns, pinned by `TestTheReleaseCarriesBothBinaries`. `TestE2EOrigoReadsCommitsAndReverts` drives every command of the surface against a running node. The live run it named has since run, on 2026-09-11 in the v0.1.3 release run 34546335576, and the refusal code its Open section reports is recorded in 020's Spec defects; 025 stays at `testing` and its own builder owns what remains |
| 13 | 026 | A browsing interface can ask which repositories a person may see, and address one by `<owner>/<slug>` instead of by identifier | complete since 2026-09-11: the two additions spec 023 of `latere-ai/origo-web` stated the shape of, a fourth action on the authorizer contract with an answer that says the authorizer has no directory, and `GET /v1/repos` in a directory mode and a name mode, are built and released in `v0.2.0`. Spec 023 lives in that repository and is `complete` since 2026-09-12: the interface is at `v0.8.0` in production with every named test in its tree, its end-to-end tier runs in a job of its own `verify.yml` against `origod:v0.2.0` by digest (first run 34657693665), and the browser half of two criteria gave way to the structural half by the user's decision |
...
|---|---|---|
| the authorizer contract has a fourth action, `list`, which carries no `repo` object and whose answer is a page, a deny, or `{"directory": false}`; spec 007's five rules and its three actions are unchanged, and an authorizer that implements only 007 stays correct | 026 | 007, 013 |
...
| the per-subject request rate is `ORIGO_REQUESTS_PER_MINUTE`, 600 by default, and every response of the surface carries the figure in force for its own subject as `RateLimit-Limit` with the tokens it has left beside it as `RateLimit-Remaining` (both IETF draft fields): the limit is the authorizer's `requests_per_minute` where it named one and the variable otherwise, and neither header is sent when the limit is off; the `kind` overlay runs at 6000 and each cluster scenario mints a subject of its own, because the scenarios drive one node far harder than any caller of a live installation, and 021's `rate_limited` case reads the header rather than assuming the default | 012 | 002, 013, 003, 021 |
...
| 021's `rate_limited` case proves the limit by `RateLimit-Remaining` over a burst of 64 requests, twice the largest replica count `deploy/base/hpa.yaml` scales to, and reads the number of buckets the target answered from off how far the counter fell; it exhausts and asserts the 429 only where one bucket answered, under a bound of twice the figure, and records the refusal in `Report.Unverified` otherwise, because a balancer gives one subject a bucket a node and no bound converges there; the stub and the stack answer from one bucket and still require that list empty | 021 | 012, 002, 013 |
...
| A key names one subject. A public key signs nothing but the session, so the key store resolves it to one subject and asserts nothing else; a machine that must push gets a subject of its own, and a service that must act for a person uses HTTPS with the token the issuer minted for that person | 024 | 007, 016 |
...
| the agent client is a command, not a tool server, and the measurement is what decided it: a registry built to the first draft's own tables serializes to 5 725 bytes of `tools/list` JSON for its eight read tools and 10 137 for all eleven, roughly 1.6 k and 2.9 k tokens, resident on every turn of every session it is configured in, used or not, against a budget of 5 KiB and 8 KiB the draft set itself. Six of the eleven tools were pipelines fused in advance because one tool call cannot pipe into another, and the two searches the draft refused as impossible are one `grep` each against a command that prints lines. What MCP buys is an agent with no shell and per-tool permissioning owned by the host, and neither is Origo's question | 025 | 003, 009 |
| `origo` is a second binary of this repository, `cmd/origo`, run on the machine the agent runs on, not a mode of `origod` and not a service of its own: `origod` gains no route, no listener, no configuration, and no threat-model surface, while the client surface moves in the same commit as the contract it is shaped from and is proved against the same stack. The split spec 023 makes for the web interface is not made here, because that is a service with a deployment, sessions, and templates, and this is a stateless client binary | 025 | 002, 017, 023 |
| the client is three packages, and the boundary is the design rather than an implementation detail: `internal/origoclient` speaks contract 1 and formats nothing, `internal/origocli` holds every flag, byte default, line writer and exit code and reaches the network only through the client, and `cmd/origo` holds `main`. A tool server, if a product ever needs one for an agent with no shell, is a fourth package on `origoclient` and inherits the routes, the paging and the refusals without inheriting one formatting decision. The first draft fused its formatting into its tool handlers, which is exactly what this prevents | 025 | - |
| data goes to stdout and every header, truncation and stale line goes to stderr. This is the rule a tool surface cannot have, and it is what makes `origo ls -r -n 0 \| grep -i handler` and `origo cat x.go > x.go` both correct: a truncation notice inside the payload would reach the filter and would land in the file. Each cut answer names on stderr the one flag that continues it, and an answer with no such line is whole | 025 | 009 |
| `origo` never mints, signs, exchanges, or refreshes a credential, and `internal/origoclient` has no method that calls `POST /v1/repos/{id}/tokens` or any route of spec 019, asserted over its exported method set rather than left to review. It carries one bearer from `ORIGO_TOKEN`, and the credential to hand it is a repository-bound token of spec 007, whose scope bounds what the agent may do and whose hour bounds a leak. An agent acting for a person is handed a token minted from the person's own, so it names the person rather than constructing a claim | 025 | 007, 012 |
| no command of `origo` can make a commit unreachable: every mutation is one of spec 020's operations, which appends a commit whose parent is `expected_head` and moves one branch to it, and no command deletes a branch, a tag, a repository, or history. Spec 019's seven-day hold therefore never applies to an agent's mistake, for two independent reasons, no command deletes and no bound token reaches an `admin` action, and the recovery path is `origo revert`, which is itself additive. The credential is the write gate, because a flag on a command line is one the caller can also decline to type, and a branch allow-list in the client is refused as a second access-control model diverging from the authorizer, the ground spec 023 refuses one on | 025 | 019, 020, 023 |
| `origo` ships in spec 017's `release-archives` beside `origod` for the same four os/arch pairs and into the same `checksums.txt`; the upload is glob-driven and needs nothing, but `release-verify` downloads by an explicit `--pattern 'origod_*.tar.gz'` and then runs `sha256sum -c checksums.txt`, so the pattern gains `origo_*.tar.gz` in the same change or the next tag fails on four sums whose files it never fetched. There is no second image: the command runs beside an agent, not in a cluster. It takes a `depcheck` row of its own that is a strict subset of the node's, `latere.ai/x/pkg` with `github.com/google/uuid` behind it reached only through `internal/contract`, and the standard library: an HTTP client over `net/http` and `encoding/json` needs no upstream root at all, unlike spec 024's `golang.org/x/crypto/ssh` | 025, 017 | 001, 002 |
...
skip traveled through `publish`, which runs under `always()`, into
...
`code.latere.ai` does not resolve, so 003, 019, 020, and 021 stay at
...
`https://code.latere.ai`". The log says otherwise: with `ORIGO_LIVE_URL`
...
(http://localhost:30080)`. The job dialed no installation. The empty
secret is the whole cause; `code.latere.ai` not resolving is a separate
...
running. The same day the round read spec 023 in `latere-ai/origo-web`
against its tree: it sat at `drafted` with "Nothing is built" while
`v0.8.0` served production; every one of its 79 named tests existed,
its end-to-end tier had never run in a job, and two criteria named a
browser half nothing drove. The tier now runs against the pinned
`origod` on every push there, the two criteria name their structural
half, and 023 is `complete`. Each is filed: the library items on `latere-ai/pkg` as the
...
those is unbuilt behavior: they are tests nobody has written and one
...
What remains, all of it, with what closes each. Six specs are at
`testing`, four having closed on 2026-09-11 against the v0.1.3 release
run 34546335576. Rows the release closed are kept with the run that
closed them, because the previous form of this table is what let four
closed specs sit unrecorded for a day.

| Item | Specs | What closes it |
|---|---|---|
| the live run of `TestContract` against a deployed Origo | 003, 019, 020, 021 | closed 2026-09-11. The `live` job of the v0.1.3 run 34546335576, id 103120952813, dialed `https://code.latere.ai` and reported `--- PASS: TestContract (173.69s)`, 51 passed, with the eight case names of exactly the six skip groups. All four specs are `complete` |
...
| `TestSlowMaterializeTenThousandEntries` | 004 | closed 2026-09-11. `efc879b` added it to `test/e2e/e2e_test.go` alongside `TestE2EHundredConcurrentPushesFromEightClients`, green in job 103373438650 of the dispatched run 34632623961 at `--- PASS (495.42s)`. Issue #10 closed 2026-09-17 as already done |
...
| `TestReadTrace` in `cmd/origod` | 009 | closed 2026-09-11. `99cd827` added it to `cmd/origod/telemetry_test.go`, reading a trace of a read through a second node with an empty disk, the one shape that carries all three of `index.check`, `materialize` and `git.<command>`. Issue #11 closed 2026-09-17 as already done |
...
- Acceptance criteria are the test list: one sentence each, naming the
  behavior, the fixture or load, the threshold, and the test that
...
| variable | `ORIGO_SSH_KEYS_URL` | [024](024-ssh-access.md) | 002, 013, 028 |
