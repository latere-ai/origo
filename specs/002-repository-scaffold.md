...
| `ORIGO_REQUESTS_PER_MINUTE` | spec 012 | `600` | the requests one effective subject may send a node in a minute, and the bucket's burst; `0` turns the per-subject limit off, which the `kind` overlay of spec 013 does because its scenarios drive one node far harder than any one caller of a live installation |
