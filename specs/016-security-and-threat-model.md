...
| a reference name, owner, or slug with a metacharacter, `..`, or a control character is refused; the fuzz finds nothing git refuses | `internal/wal`, `TestValidRefName`, `TestValidLabel`, `FuzzValidRefName`, `FuzzValidLabel`; `make fuzz` runs both for 40 seconds |
...
| the egress dialer on its own | `internal/api`, `TestEgressDialerHonoursTheAllowList`, `TestEgressDialerAdmitsOnlyThePinnedClusterAddress`, `TestEgressDialerAllowLoopbackIsATestSeam`, `TestEgressDialerPinsTheResolvedAddress`; `internal/config`, `TestEgressAllowPinsOnlyExactHosts`, `TestEgressCABundleIsReadAtStartup` |
...
- `TestClusterPodSecurityContext` asserts the memory request and limit
  of the Design and that a CPU request is set, because spec 005's kind
  overlay runs the stack at 50m and the base at 250m; the criterion
  reads 250m or 50m, and the test's assertion at the two figures is
  left to spec 019's builder, who owns `test/e2e` this round.
...
(specs 019, 014), the way specs 003 and 004 wait, plus the CPU figure
assertion left to spec 019's builder.
Coverage at `5fa8275`: `internal/api` 93.1%, `internal/auth` 96.4%,
`internal/config` 99.4%, `internal/httpgit` 93.8%, `internal/repo`
93.6%, `internal/wal` 97.8%, `cmd/origod` 94.8%. A second assertion is
left to spec 019's builder, who owns `internal/wal` this round:
`TestValidLabel` lists `a..b` in neither of its two lists where the
criterion says it is admitted, and `FuzzValidLabel` seeds it but holds
only that git accepts what the validator accepts.

Open, for the next round to decide: a pinned address outside every
range of `ORIGO_CLUSTER_CIDRS` admits nothing, because the dialer
judges it by the well-known ranges alone, refusing it when private
and admitting the host without the pin when public. Two candidate
rules. First, the pin is inert and therefore a problem of the one
start-up message and of `origod check` (spec 018): `host=address`
exists to admit one in-cluster address, so an address no listed range
contains is a configuration the operator did not mean, and with
`ORIGO_CLUSTER_CIDRS` unset every pin is refused, which tells an
operator that the ranges are what a pin needs. Second, the pin is
admitted as it is and does nothing, the tree today, which keeps a
configuration valid across a change of the cluster's ranges at the
cost of a pin that silently stops admitting. The opposite rule, that a
pin inside a cluster range is the error, does not apply: the ranges
are what a fetch must never reach and the pin is the one exception to
them (this spec's egress row, spec 002's rows), and the kind overlay's
`origo-stubs.origo.svc=10.96.0.42` inside kind's service range is the
pin's own use.
