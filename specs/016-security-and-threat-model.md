...
- The egress dialer, tested on its own in `internal/api` with no
  ...
  refused; with `AllowLoopback` set through the constructor a listed
  host on `127.0.0.1` is dialed and an unlisted one is still refused;
