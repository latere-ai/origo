...
|---|---|---|
| `test/stubs/issuer` | the family's stub issuer, `latere.ai/x/pkg/authkit/issuertest`, with Origo's defaults: ES256 keys and the audience `origo`. The shared stub serves the discovery document, `/jwks`, `POST /mint` for a token with any one claim wrong, `POST /actor-tokens` as the family's one hop (latere-ai/specs `infrastructure/identity/id-03-one-hop.md`), `/rotate`, `/hang` and `/resume`; it moved out of this repository with id-04 so every repository's tests and the conformance suite run one issuer. It serves plain HTTP, which is why the node lists it in `ORIGO_OIDC_INSECURE_ISSUERS` (spec 007) wherever its host is not a loopback address | `Mint(claims) string`, `Rotate()`, `Hang()`, `Resume()`, `URL()`, `Delegated(sub, act)` |
