...
|---|---|---|
| clone, fetch, push, and every capability of spec 003's table | unchanged | the same subprocess, the same advertisement, the same `atomic` and `push-options` behavior |
...
| Secret `origod-ssh-host-key` | no template, like `origod-token-key`: generated by the operator through the install document's SSH step, and named in `deploy/bootstrap/README.md` | the host keys; never in `deploy/base`, for the reason spec 018 gives for every Secret |
...
| decision 9: the size limits of spec 012 are "unchanged" and measured before git runs | the repository quota is measured when the pre-receive hook hands the transaction over, not before the subprocess starts. A stream carries no length, so the pack's size is not known until the client has sent it. The refusal is still the hook's verdict, so no entry is written and the client reads the code and the sentence in the sideband, which is the behavior the HTTP path has. The single-push bound is enforced as the bytes pass, before the pack is indexed |
