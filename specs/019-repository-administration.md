...
- After 500 pushes and a `gc`, `stats.size_bytes` is within 10% of the
  pack size of a fresh `git clone --mirror`, which holds because the
  `compact` entry the `gc` commits sets `size_bytes` to the bytes of
  the packs it lists (spec 004) rather than leaving the 500 pushes'
  bytes in the sum, and the weekly sweep run
