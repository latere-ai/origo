...
var kindOrder = []Kind{KindCode, KindVariable, KindMetric, KindEvent, KindEndpoint, KindHeader}
...
func Build(dir string) (*Index, error) {
	...
	for _, m := range mentions {
		if m.kind == "" {
			...
			// definitions: an error code or an event kind.
			for _, k := range []Kind{KindCode, KindEvent} {
...
func scan(num, body string) ([]definition, []mention) {
	...
	for _, p := range backtick.FindAllStringSubmatch(text, -1) {
		tok := p[1]
		// A metric named with its labels, origo_x_total{result="error"},
		// mentions the metric.
