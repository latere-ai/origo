...
// name's words, linked from docs/internals/ into specs/.
...
const (
	...
	authzIntro   = "Every path above is one Origo serves. This is the one call it makes: to the authorization endpoint the operator runs, which decides every repository operation. The same contract written for whoever implements an endpoint is in [`docs/api.md`](../api.md), and an operator installing one starts at [`docs/install.md`](../install.md)."
)
...
// Page renders docs/internals/contract.md from the index.
func Page(idx *specs.Index) (string, error) {
	...
	var b strings.Builder
	fmt.Fprintf(&b, `# Origo contract reference

For whoever changes Origo. This page is generated by %s from the
endpoint, header, and code tables of the design specs, and each table
links the spec that owns it, so a name on this page is a name the
implementation is held to. Run it again after a spec changes; the
specindex job fails a push whose page is not what a fresh run produces.

Whoever builds a client reads [`+"`docs/api.md`"+`](../api.md) instead. It
describes the same surface in terms of what a caller does, and a test
holds it to every path and error code this page carries. This page is
the design detail behind each row: the git command a read runs, the
bound it is held to, and the reason for each.
...
Every route but the probes, the landing page, the key set, and the
OpenAPI document is authenticated. A request carries
`+"`Authorization: Bearer <token>`"+`, including on the git routes, where a
Basic password is read as the same bearer. The read routes also admit a
request with no credential when the installation sets
`+"`ORIGO_ANONYMOUS_READ`"+`; [`+"`docs/configuration.md`"+`](../configuration.md)
lists them.
