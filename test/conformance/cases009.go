...
func case009Refs(t *testing.T, s *session) {
	...
	check(t, !(byName["refs/heads/main"]["sha"] != f.c2 || byName["refs/tags/v1"]["sha"] != f.tag || byName["refs/tags/v1"]["peeled"] != f.tagged), "refs: %s", r.body)
	r = s.read(t, "/refs?prefix=refs/tags/", nil)
	decode(t, r.body, &refs)
	check(t, !(len(refs) != 1 || refs[0]["name"] != "refs/tags/v1" || r.header.Get(api.HeaderTruncated) != ""), "tags: %s", r.body)
}

func case009Commits(t *testing.T, s *session) {
	f := s.fixture
	r := s.read(t, "/commits?limit=2", nil)
	check(t, !(r.header.Get(api.HeaderCommit) != f.c2), "%s %q", api.HeaderCommit, r.header.Get(api.HeaderCommit))
	commits, _ := r.json["commits"].([]any)
	check(t, !(len(commits) != 2 || r.json["next_cursor"] == nil), "first page: %s", r.body)
	first, _ := commits[0].(map[string]any)
	check(t, !(first["sha"] != f.c2 || first["message"] == nil || first["author"] == nil || first["committer"] == nil || first["parents"] == nil), "commit shape: %v", first)
	...
	commits, _ = r.json["commits"].([]any)
	check(t, !(len(commits) != 1 || obj(commits[0])["sha"] != f.c1 || r.json["next_cursor"] != nil), "second page: %s", r.body)
	r = s.read(t, "/commits?ref=v1", nil)
	commits, _ = r.json["commits"].([]any)
	check(t, len(commits) == 2, "by tag: %s", r.body)
	...
	expectStatus(t, r, http.StatusOK)
	commits, _ = r.json["commits"].([]any)
	check(t, len(commits) == 0 && r.json["next_cursor"] == nil, "empty repository: %s", r.body)
}
...
func case009Commit(t *testing.T, s *session) {
	...
	check(t, !(r.json["sha"] != f.c2 || stats["files"] != float64(1) || stats["additions"] != float64(1) || stats["deletions"] != float64(0)), "commit: %s", r.body)
	// ETag revalidation.
	...
	check(t, !(r.header.Get("ETag") == "" || r2.status != http.StatusNotModified), "ETag %q: %d", r.header.Get("ETag"), r2.status)
}

func case009Compare(t *testing.T, s *session) {
	f := s.fixture
	r := s.read(t, "/compare/"+f.c1+"..."+f.c2, nil)
	check(t, !(!strings.HasPrefix(r.header.Get("Content-Type"), "text/x-diff") || !strings.Contains(string(r.body), "+++ b/sub/b.txt") || !strings.Contains(string(r.body), "Binary files")), "compare: %q\n%s", r.header.Get("Content-Type"), r.body)
	r = s.read(t, "/compare/"+f.c1+"..."+f.c2+"?path=sub", nil)
	check(t, !(strings.Contains(string(r.body), "bin.dat")), "path filter: %s", r.body)
}
...
func case009Tree(t *testing.T, s *session) {
	...
	check(t, !(len(entries) != 3), "tree: %s", r.body)
...
func case009Blob(t *testing.T, s *session) {
	...
	check(t, !(r.status != http.StatusPartialContent || string(r.body) != "two" || r.header.Get("Content-Range") != "bytes 4-6/8"), "range: %d %q %q", r.status, r.body, r.header.Get("Content-Range"))
}
...
func case009Archive(t *testing.T, s *session) {
	...
	check(t, !(first.header.Get("Content-Type") != "application/gzip" || !bytes.Equal(first.body, second.body)), "archive: %q, %d and %d bytes", first.header.Get("Content-Type"), len(first.body), len(second.body))
	...
	for _, e := range entries {
		check(t, !(!strings.Contains(listed, prefix+e)), "archive lacks %s%s:\n%s", prefix, e, listed)
	}
	check(t, !(strings.Contains(listed, ".git/")), "archive carries .git:\n%s", listed)
}
...
func case009RefNotFound(t *testing.T, s *session) {
	...
	check(t, !(d["ref"] != "refs/heads/nope"), "details: %v", d)
...
func case009BlobTooLarge(t *testing.T, s *session) {
	...
	check(t, !(d["size"] != float64(api.MaxBlobBytes+1) || d["max"] != float64(api.MaxBlobBytes)), "details: %v", d)
	r := s.do(t, request{method: "GET", path: "/v1/repos/" + id + "/blob/" + blob, token: s.target.Token, header: map[string]string{"Range": "bytes=0-1023"}})
	check(t, !(r.status != http.StatusPartialContent || len(r.body) != 1024), "range of the large blob: %d %d bytes", r.status, len(r.body))
}
