...
var Groups = []Group{{
	...
	Variables: []Variable{
		...
		{"ORIGO_OIDC_AUDIENCE", "no", "`origo`", "the audiences an accepted token may carry, comma separated; a token is accepted when its `aud` names any of them. The first entry is the primary, and the repository-bound tokens Origo mints carry that one. The default is the service's own name, and the deployment manifests name it explicitly rather than inherit it; set it when the issuer mints for a different audience, or name a second entry when one node answers at a second address. An empty or repeated entry is a start-up failure."},
