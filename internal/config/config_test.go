...
// hostKeyFile writes a private key in a form ssh.ParsePrivateKey reads:
// a PKCS#8 PEM block, which is what a key generated by any tool other
// than ssh-keygen looks like. The package under test parses it through
// internal/sshd, so the test needs no SSH library of its own.
func hostKeyFile(t *testing.T, name string, key any) string {
	t.Helper()
...
func ed25519HostKey(t *testing.T) string {
	t.Helper()
	_, key, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	return hostKeyFile(t, "ed25519", key)
}

// TestSSHConfigurationIsAllOrNothing is spec 024's start-up criterion:
// ORIGO_SSH_ADDR set with any of the other three unset names every
// missing one in the one message, a host key file that does not parse
// and a key of an algorithm that is not a host key algorithm are
// problems of the same message, and ORIGO_SSH_ADDR unset starts a node
// with none of them set and opens no listener.
func TestSSHConfigurationIsAllOrNothing(t *testing.T) {
	t.Run("unset is off", func(t *testing.T) {
		cfg, err := Load(env(complete(t)))
		if err != nil {
			t.Fatal(err)
		}
		if cfg.SSHAddr != "" || cfg.SSHHostKeys != nil || cfg.SSHKeysURL != "" || cfg.SSHKeysToken != "" {
			t.Errorf("SSH is configured with ORIGO_SSH_ADDR unset: %+v", cfg)
		}
	})

	t.Run("every missing variable in one message", func(t *testing.T) {
		m := complete(t)
		m["ORIGO_SSH_ADDR"] = ":2222"
		_, err := Load(env(m))
		if err == nil {
			t.Fatal("expected an error")
		}
		for _, key := range []string{"ORIGO_SSH_HOST_KEYS", "ORIGO_SSH_KEYS_URL", "ORIGO_SSH_KEYS_TOKEN"} {
			if !strings.Contains(err.Error(), "missing "+key) {
				t.Errorf("message %q does not name %s", err, key)
			}
		}
	...
	t.Run("a URL that is not absolute", func(t *testing.T) {
		m := complete(t)
		m["ORIGO_SSH_ADDR"] = ":2222"
		m["ORIGO_SSH_HOST_KEYS"] = ed25519HostKey(t)
		m["ORIGO_SSH_KEYS_URL"] = "keys.example"
		m["ORIGO_SSH_KEYS_TOKEN"] = "s"
		if _, err := Load(env(m)); err == nil || !strings.Contains(err.Error(), "ORIGO_SSH_KEYS_URL") {
			t.Fatalf("a relative key resolution URL was accepted: %v", err)
		}
	})

	t.Run("a host key that does not parse", func(t *testing.T) {
		...
		m := complete(t)
		m["ORIGO_SSH_ADDR"] = ":2222"
		m["ORIGO_SSH_HOST_KEYS"] = bad + "," + filepath.Join(t.TempDir(), "missing")
		m["ORIGO_SSH_KEYS_URL"] = "https://keys.example"
		m["ORIGO_SSH_KEYS_TOKEN"] = "s"
		_, err := Load(env(m))
		if err == nil || !strings.Contains(err.Error(), "ORIGO_SSH_HOST_KEYS") || !strings.Contains(err.Error(), bad) {
			t.Fatalf("a host key that does not parse was accepted: %v", err)
		}
	})
	...
	t.Run("an RSA key below the floor", func(t *testing.T) {
		...
		m := complete(t)
		m["ORIGO_SSH_ADDR"] = ":2222"
		m["ORIGO_SSH_HOST_KEYS"] = hostKeyFile(t, "rsa", key)
		m["ORIGO_SSH_KEYS_URL"] = "https://keys.example"
		m["ORIGO_SSH_KEYS_TOKEN"] = "s"
		if _, err := Load(env(m)); err == nil || !strings.Contains(err.Error(), "at least 2048 bits") {
			t.Fatalf("a 1024-bit RSA host key was accepted: %v", err)
		}
	})
	...
	t.Run("a complete set loads", func(t *testing.T) {
		...
		m["ORIGO_SSH_ADDR"] = ":2222"
		m["ORIGO_SSH_HOST_KEYS"] = ed25519HostKey(t)
		m["ORIGO_SSH_KEYS_URL"] = "https://keys.example/internal/origo/ssh-keys"
		m["ORIGO_SSH_KEYS_TOKEN"] = "s"
		cfg, err := Load(env(m))
		if err != nil {
			t.Fatal(err)
		}
		if cfg.SSHAddr != ":2222" || cfg.SSHHostKeys == nil || len(cfg.SSHHostKeys.All()) != 1 || cfg.SSHKeysToken != "s" {
